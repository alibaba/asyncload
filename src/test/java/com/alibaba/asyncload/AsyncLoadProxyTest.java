...
public class AsyncLoadProxyTest extends BaseAsyncLoadNoRunTest {
	...
	public void testProxy_timeout() {
		...
		AsyncLoadTestModel model1 = service.getRemoteModel("first", 1000); // 每个请求sleep
																			// 1000ms
		AsyncLoadTestModel model2 = service.getRemoteModel("two", 200); // 每个请求sleep
																		// 1000ms
	...
	public void testProxy_block_reject() {
		...
		// 初始化executor
		AsyncLoadExecutor executor = new AsyncLoadExecutor(8, 2, PoolRejectHandleMode.REJECT); // 设置为拒绝,8个工作线程,2个等待队列
		executor.initital();
		// 初始化proxy
		AsyncLoadEnhanceProxy<AsyncLoadTestService> proxy = new AsyncLoadEnhanceProxy<AsyncLoadTestService>();
		proxy.setService(asyncLoadTestService);
		...
		try {
			for (int i = 0; i < 10; i++) { // 创建10个任务
				final AsyncLoadTestModel model = service.getRemoteModel("first:" + i, 1000); // 每个请求sleep
																								// 1000ms
	...
	public void testProxy_block_reject_noQueue() {
		...
		// 初始化executor
		AsyncLoadExecutor executor = new AsyncLoadExecutor(2, 0, PoolRejectHandleMode.REJECT); // 设置为拒绝
		executor.initital();
		// 初始化proxy
		AsyncLoadEnhanceProxy<AsyncLoadTestService> proxy = new AsyncLoadEnhanceProxy<AsyncLoadTestService>();
		proxy.setService(asyncLoadTestService);
	...
	public void testProxy_block_callerRun() {
		...
		AsyncLoadConfig config = new AsyncLoadConfig(3 * 1000l); // 设置超时时间为3000ms
		// 初始化executor
		AsyncLoadExecutor executor = new AsyncLoadExecutor(1, 0, PoolRejectHandleMode.CALLERRUN); // 设置为caller线程运行模式,10个工作线程,0个等待队列
		executor.initital();
		// 初始化proxy
		AsyncLoadEnhanceProxy<AsyncLoadTestService> proxy = new AsyncLoadEnhanceProxy<AsyncLoadTestService>();
		proxy.setService(asyncLoadTestService);
